...
Die für Sie verfügbaren Funktionen hängen davon ab, ob Sie der *Ersteller* (die Person, von der die Karte erstellt wurde) oder der *Nutzer* (jemand hat eine ArcGIS-Karte für Sie freigegeben) sind. Wenn Sie als Nutzer mit einer ArcGIS-Karte interagieren (auch als [Leseansicht](end-user-reading-view.md) bezeichnet), sind die folgenden Aktionen für Sie verfügbar.

* Als Premium-Nutzer mit *Anzeigeberechtigungen* können Sie [die Daten zum Erstellen der Visualisierung anzeigen](end-user-show-data.md) , [abonnieren](end-user-subscribe.md), die Karte im [Fokusmodus und Vollbildmodus anzeigen](end-user-focus.md), [zugehörigen Inhalt anzeigen](end-user-related.md), [mit den Filtern interagieren](end-user-report-filter.md), die vom *Berichtersteller* festgelegt wurden, [den Bericht freigeben](../service-share-reports.md) und mehr.
